...
export function HtpDrawingBoard({ clientId }: HtpDrawingBoardProps) {
  ...
  const resultHref = clientId ? `/admin/clients/curriculum/htp/result?clientId=${clientId}` : "";
  ...
  useEffect(() => {
    ...
    const loadSavedResult = async () => {
      try {
        const response = await fetch(`/api/clients/${clientId}/tests/htp`, {
  ...
  return (
    <div className="mt-6 space-y-4">
      ...
      <div className="rounded-xl border border-slate-300 bg-white p-1.5 sm:p-2">
        <div
          ref={boardRef}
          className="relative w-full overflow-hidden rounded-md border border-slate-200 aspect-[4/3] sm:aspect-[16/10] lg:aspect-[16/9]"
        >
          <canvas
            ref={canvasRef}
