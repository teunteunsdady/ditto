"use client";

import Image from "next/image";
import Link from "next/link";
import { type CSSProperties } from "react";
import coregroundLogo from "../../public/images/coreground-logo.png";

type CompanyScrollSectionsProps = {
  includeIntroSection?: boolean;
  includeMiddleSections?: boolean;
  includeContactSection?: boolean;
  introSectionId?: string;
  contactSectionId?: string;
};

export function CompanyScrollSections({
  includeIntroSection = true,
  includeMiddleSections = true,
  includeContactSection = true,
  introSectionId = "about",
  contactSectionId = "proof",
}: CompanyScrollSectionsProps) {
  const manifestoSlides = [
    {
      label: "THE PROBLEM",
      title: "부유(浮遊)하는 세대",
      description:
        "우리는 그 어느 때보다 연결되어 있지만, 동시에 가장 쉽게 흔들리는 시대를 살고 있습니다.\n타인의 SNS, 사회가 정한 성공의 기준, 끊임없이 쏟아지는 외부의 소음 속에서 2030의 자아는 마치 공중에 붕 뜬 것처럼 불안하게 흔들립니다.",
    },
    {
      label: "THE SOLUTION",
      title: "단단한 착지 : 온전한 자립 (自立)",
      description:
        "CoreGround는 질문합니다. \"지금 당신은 남의 지배나 부축을 받지 않고 자기 힘으로 온전히 서 있나요?\" 인문학적 통찰과 탐구를 통해 자신이 가진 고유한 핵심(Core)을 발견하고 그것을 삶의 단단한 지반(Ground)으로 만드는 정서적 근력을 기르고자 합니다.",
    },
    {
      label: "THE GOAL",
      title: "주체성 (主體性)",
      description:
        "세상의 시선이 아닌, 나만의 시선과 내면의 목소리에 집중할 때 우리는 성장을 배웁니다.  CoreGround는 어제보다 성장한 나를 통해 세상의 폭풍 속에서 자신을 잃지 않고 당당히 걸어 나갈 수 있는 세상을 꿈꿉니다.",
    },
  ];

  const keywords = [
    "정서적 근력",
    "내면의 기준점",
    "자기 이해에서 실행까지",
    "흔들리지 않는 중심",
    "청년 성장 설계",
    "코칭 기반 AI 키트",
  ];

  const programs = [
    {
      title: "개인 프로그램",
      detail: "자기 이해와 실행에 집중하는 1:1 코칭 ",
    },
    {
      title: "그룹 프로그램",
      detail: "더욱 다채로운 소통방식의 습득과 협업 능력을 개발하는 팀/집단 코칭",
    },
    {
      title: "온보딩 프로그램",
      detail: "새로운 환경 적응과 첫 100일 성장을 설계하는 프로그램",
    },
  ];

  return (
    <>
      {includeIntroSection ? (
        <section id={introSectionId} className="relative overflow-hidden border-b border-[#e7ebe8] bg-white">
        <div className="pointer-events-none absolute right-[-4.5rem] top-[-3rem] h-72 w-72 opacity-[0.06] sm:right-[-3rem] sm:h-96 sm:w-96">
          <Image
            src={coregroundLogo}
            alt=""
            fill
            sizes="384px"
            className="object-contain"
            placeholder="blur"
          />
        </div>
        <div className="mx-auto w-full max-w-6xl px-4 pb-14 pt-12 sm:px-6 sm:pb-16 sm:pt-16 lg:pb-20 lg:pt-20">
          <h1 className="mt-5 text-3xl font-semibold leading-[1.18] tracking-tight sm:text-5xl lg:text-6xl">
            지금 &apos;CoreGround&apos;가 왜 필요한가
          </h1>
          <p className="mt-7 max-w-3xl whitespace-pre-line text-base leading-relaxed text-[#4e5968] sm:text-lg">
            {"풍요 속의 빈곤을 이야기하는 시대,\nCoreGround는 위로를 넘어, 개인의 핵심을 삶의 지반으로 만드는 정서적 근력을 설계합니다."}
          </p>
        </div>
      </section>
      ) : null}

      {includeMiddleSections ? (
        <>
      <section id="expertise-1" className="mx-auto w-full max-w-6xl space-y-3 px-4 py-6 sm:px-6 sm:py-8">
          {manifestoSlides.map((item, index) => (
            <article
              key={item.title}
              data-reveal
              className="cg-rotate-reveal-card relative overflow-hidden rounded-[28px] border border-[#dfe6e3] bg-white px-6 py-9 shadow-[0_14px_36px_rgba(15,23,42,0.06)] sm:px-10 sm:py-11"
              style={
                {
                  "--card-tilt-start": `${index % 2 === 0 ? "-5deg" : "5deg"}`,
                } as CSSProperties
              }
            >
              <div className="pointer-events-none absolute right-4 top-4 h-20 w-20 opacity-[0.045] sm:h-28 sm:w-28">
                <Image
                  src={coregroundLogo}
                  alt=""
                  fill
                  sizes="112px"
                  className="object-contain"
                  placeholder="blur"
                />
              </div>
              <p className="text-xs font-semibold tracking-[0.16em] text-[#6b7684]">
                {item.label}
              </p>
              <h2 className="mt-4 max-w-3xl text-3xl font-semibold leading-tight tracking-tight sm:text-4xl lg:text-5xl">
                {item.title}
              </h2>
              <p className="mt-8 max-w-4xl whitespace-pre-line text-base leading-relaxed text-[#333d4b] sm:text-lg sm:leading-8">
                {item.description}
              </p>
            </article>
          ))}
      </section>

      <section id="expertise-2" className="border-y border-[#e7ebe8] bg-[#f8faf9]">
        <div className="mx-auto grid w-full max-w-6xl gap-8 px-4 py-14 sm:px-6 sm:py-16 lg:grid-cols-[1.15fr_0.85fr] lg:py-20">
          <div data-reveal className="cg-vision-reveal">
            <p className="text-xs font-semibold tracking-[0.16em] text-[#6b7684]">VISION</p>
            <h2 className="mt-4 text-3xl font-semibold leading-tight tracking-tight sm:text-4xl lg:text-5xl">
              코칭과 AI를 접목한
              <br />
              실천형 성장 키트 개발
            </h2>
            <p className="mt-6 max-w-3xl text-base leading-relaxed text-[#4e5968] sm:text-lg">
              진단-실행-회고를 한 회차로, 자신의 상태를 점검하고 돕는 코칭 AI 키트를 만듭니다.
            </p>
          </div>
          <div data-reveal className="cg-vision-reveal rounded-3xl border border-[#dde5e2] bg-white p-6 sm:p-7">
            <p className="text-xs font-semibold tracking-[0.16em] text-[#6b7684]">KEY KEYWORDS</p>
            <div className="mt-4 flex flex-wrap gap-2">
              {keywords.map((keyword) => (
                <span
                  key={keyword}
                  className="rounded-full border border-[#dbe3e0] bg-[#f5f8f7] px-3 py-1.5 text-xs font-semibold text-[#2f4f46] sm:text-sm"
                >
                  {keyword}
                </span>
              ))}
            </div>
          </div>
        </div>
      </section>

      <section id="process" className="mx-auto w-full max-w-6xl px-4 py-14 sm:px-6 sm:py-16 lg:py-20">
        <div data-reveal>
          <p className="text-xs font-semibold tracking-[0.16em] text-[#6b7684]">PROGRAMS</p>
          <h3 className="mt-4 text-3xl font-semibold tracking-tight sm:text-4xl">개인 · 그룹 · 온보딩</h3>
          <div className="mt-8 grid gap-4 sm:grid-cols-3">
            {programs.map((item, index) => (
              <article
                key={item.title}
                data-reveal
                className="cg-program-rotate-card rounded-2xl border border-[#e2e7e5] bg-white p-6"
                style={
                  {
                    "--program-tilt-start": `${index % 2 === 0 ? "-6deg" : "6deg"}`,
                  } as CSSProperties
                }
              >
                <h4 className="text-xl font-semibold tracking-tight">{item.title}</h4>
                <p className="mt-3 text-sm leading-relaxed text-[#4e5968] sm:text-base">{item.detail}</p>
              </article>
            ))}
          </div>
        </div>
      </section>
      </>
      ) : null}

      {includeContactSection ? (
        <section id={contactSectionId} className="border-t border-[#e7ebe8] bg-white">
        <div className="mx-auto w-full max-w-6xl px-4 py-14 sm:px-6 sm:py-16 lg:py-20">
          <div className="rounded-3xl bg-[#2f4f46] px-6 py-10 text-white sm:px-10 sm:py-12">
            <p className="text-xs font-semibold tracking-[0.16em] text-[#d7e8e1]">CONTACT</p>
            <h2 className="mt-4 text-3xl font-semibold leading-tight tracking-tight sm:text-4xl">
              현재 나의 중심은 어디를 향하고 있나요?
            </h2>
            <p className="mt-4 max-w-2xl text-sm leading-relaxed text-[#e6f1ec] sm:text-base">
              현재 고민과 목표를 남겨주시면 개인, 그룹, 신입생 중 가장 적합한 코칭 흐름으로 빠르게
              안내해드립니다.
            </p>
            <Link
              href="/contact"
              className="mt-7 inline-flex h-11 items-center justify-center rounded-xl bg-white px-6 text-sm font-semibold text-[#223c35] transition-colors hover:bg-[#edf4f1]"
            >
              문의하기
            </Link>
          </div>
        </div>
      </section>
      ) : null}
    </>
  );
}
